...
void hexify(const unsigned char* buffer, size_t length)
{
...
int aes_example()
{
	...
	std::string ciphertext = aes.encrypt(plaintext.c_str(), static_cast<uint32_t>(plaintext.length()));
	...
	std::string decrypttext = aes.decrypt(ciphertext.c_str(), static_cast<uint32_t>(ciphertext.length()));
